...
public class MemberParser implements ArgumentParser<CommandSender, OfflinePlayer>, SuggestionProvider<CommandSender> {
	...
	public @NonNull ArgumentParseResult<@NonNull OfflinePlayer> parse(@NonNull CommandContext<@NonNull CommandSender> commandContext, @NonNull CommandInput commandInput) {
		...
		if (!(commandContext.sender() instanceof Player player)){
			return ArgumentParseResult.failure(new TranslationException(OfflinePlayer.class, commandContext, Translations.ARGUMENT_CANNOT_USE_COMMAND));
		}

		if (unity.getPlayerManager().fromBukkit(player).getFactionId() == null){
			return ArgumentParseResult.failure(new TranslationException(OfflinePlayer.class, commandContext, Translations.ARGUMENT_REQUIRE_FACTION));
		}
		...
		if (!faction.isMember(offlinePlayer.getUniqueId())){
			return ArgumentParseResult.failure(new TranslationException(OfflinePlayer.class, commandContext, Translations.ARGUMENT_PLAYER_NOT_IN_SAME_FACTION));
		}
